...
Connects to Kafka brokers and consumes one or more topics.`,
		Description: `
Offsets are managed within Kafka as per the consumer group (set via config), and partitions are automatically balanced across members of the consumer group.

Use the ` + "`batching`" + ` fields to configure an optional [batching policy](/docs/configuration/batching#batch-policy). Unless [parallel processing](#parallel_processing) is enabled any other batching mechanism will stall with this input due its sequential transaction model.
...
You can access these metadata fields using [function interpolation](/docs/configuration/interpolation#metadata).`,
		...
		FieldSpecs: docs.FieldSpecs{
			...
			docs.FieldCommon(
				"topics",
				"A list of topics to consume from. Multiple comma separated topics can be listed in a single element. Partitions are automatically distributed across consumers of a topic. Alternatively, it's possible to specify explicit partitions to consume from with colons after the topic name, e.g. `foo:0:2` would consume partitions 0 and 2 of the topic foo.",
				[]string{"foo", "bar"}, []string{"foo,bar"}, []string{"foo:0", "bar:1:3"}, []string{"foo:0,bar:1:3"},
			),
			btls.FieldSpec(),
			sasl.FieldSpec(),
			...
			docs.FieldAdvanced("checkpoint_limit", "EXPERIMENTAL: Allows N messages of the same topic and partition to be processed asynchronously, this enables parallel processing and batching at the output level to work on individual partitions. Any given messages offset will not be committed unless all messages under that offset are delivered, therefore out of order processing can increase the likelihood of duplicated messages during service crashes."),
		...
		for _, splitTopics := range strings.Split(t, ",") {
			if trimmed := strings.TrimSpace(splitTopics); len(trimmed) > 0 {
				if withParts := strings.Split(trimmed, ":"); len(withParts) > 1 {
					...
					for _, pStr := range withParts[1:] {
						partition, err := strconv.ParseInt(pStr, 10, 32)
						if err != nil {
							return nil, fmt.Errorf("failed to parse partition number: %w", err)
						}
						partitions = append(partitions, int32(partition))
					}
					k.topicPartitions[topic] = append(k.topicPartitions[topic], partitions...)
				} else {
