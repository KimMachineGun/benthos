...
EXPERIMENTAL: Allows N messages of the same topic and partition to be processed asynchronously, this enables parallel processing and batching at the output level to work on individual partitions. Any given messages offset will not be committed unless all messages under that offset are delivered, therefore out of order processing can increase the likelihood of duplicated messages during service crashes.
